...
async function narratorBroadcastAndWait(message, isNight = false) {
    ...
    // When narration is off the client suppresses display/TTS — no need to wait for speech
    const wait = aiOptions.aiNarrationEnabled ? calculateTTSDuration(message) : 150;
...
io.on('connection', (socket) => {
    ...
    socket.on('host_force_next', () => {
        ...
        clearTimeout(phaseTimeoutId);
        clearTimeout(nightTimerTimeout);
        // INTRO: advance through sub-steps (role read → mafia open → mafia close)
        if (gameState === 'INTRO') {
            if (introPendingStep) { const step = introPendingStep; introPendingStep = null; step(); }
            else startInterrogationPhase();
        }
        else if (gameState === 'DAY') startDayVoting();
        else if (gameState === 'DAY_VOTE') tallyDayVotes();
        else if (gameState === 'INTERROGATION') nextInterrogationTurn();
        else if (gameState === 'NIGHT') {
            // resolve night if sequence is empty (manual role prompting OR narration off)
            if (nightSequence.length === 0 && !nightTurnActive) {
                resolveNight();
                return;
            }
    ...
    // PHASE: INTRODUCTION
    // ------------------------------------------
    // Stores the pending intro sub-step so host_force_next can advance manually
    let introPendingStep = null;

    async function startIntroductionPhase() {
        gameState = 'INTRO';
        introPendingStep = null;
        ...
        const roleReadTime = aiOptions.manualPhaseProgression ? 0 : 5;
        if (roleReadTime > 0) { phaseEndTime = Date.now() + roleReadTime * 1000; io.emit('start_intro_timer', roleReadTime); }

        // Step A: open mafia window
        const doMafiaOpen = async () => {
            introPendingStep = null;
            clearTimeout(phaseTimeoutId);
            await narratorBroadcastAndWait('Everyone, close your eyes. Mafia, open your eyes.', true);
            const mafiaTime = aiOptions.manualPhaseProgression ? 0 : 15;
            if (mafiaTime > 0) { phaseEndTime = Date.now() + mafiaTime * 1000; io.emit('start_intro_timer', mafiaTime); }
            const mafiaRoster = Object.values(players).filter(p => isMafiaTeam(p.role)).map(p => `${p.name} (${getDisplayRole(p)})`).join(', ');
            io.to('mafia_room').emit('open_mafia_intro_chat', { roster: mafiaRoster, time: mafiaTime || 15 });

            // Step B: close mafia window and start interrogation
            const doMafiaClose = async () => {
                introPendingStep = null;
                clearTimeout(phaseTimeoutId);
            ...
            if (aiOptions.manualPhaseProgression) {
                introPendingStep = doMafiaClose;
            } else {
                phaseTimeoutId = setTimeout(doMafiaClose, mafiaTime * 1000);
            }
        };

        if (aiOptions.manualPhaseProgression) {
            introPendingStep = doMafiaOpen;
        } else {
            phaseTimeoutId = setTimeout(doMafiaOpen, roleReadTime * 1000);
        }
    }
...
server.listen(3000, () => console.log('Mafia x Clue server running...'));require('dotenv').config();
const express = require('express');
const http = require('http');
const { Server } = require('socket.io');
const path = require('path');
